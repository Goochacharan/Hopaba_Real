...
const ListingImageCarousel: React.FC<ListingImageCarouselProps> = ({
  ...
  return <div className={cn("relative group", className)}>
      <AspectRatio ratio={4 / 5} className="bg-muted">
        <img src={imageArray[currentImageIndex]} alt={`Product image ${currentImageIndex + 1}`} className={cn("w-full cursor-pointer", imageHeightClass, "object-cover" // Always use object-cover for proper fitting
      )} onClick={handleImageClick} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd} />
        
        <Button size="icon" variant="secondary" onClick={toggleWishlist} className="">
        ...
        <Badge variant="outline" className="absolute bottom-2 right-2 z-10 flex items-center gap-1 text-amber-600 bg-amber-50/90 shadow-sm border-amber-200">
